...
Public Class AccessRight
    ...
    Public Function InsertAccessRight(ByVal CODE_PROFIL As String, ByVal NOM_PROFIL As String,
        ...
        ByVal CAISSE_ENREGISTREUSE As Integer,
        ByVal FAST_FOOD As Integer,
        ByVal SERVEUR As Integer,
        ByVal BAR_FAST_FOOD As Integer,
        ByVal RESTAURANT_FAST_FOOD As Integer,
        ByVal MENU_MARKETING As Integer,
        ByVal MARK_CARDEX As Integer,
        ByVal MARK_RAPPORT As Integer,
        ByVal MARK_DASH As Integer,
        ByVal MARK_MESSAGERIE As Integer,
        ByVal MARK_PLANNIFICATION As Integer,
        ByVal MARK_CLUB_ELITE As Integer) As Boolean

        Dim insertQuery As String = "INSERT INTO `utilisateur_acces`(`CODE_PROFIL`, `NOM_PROFIL`, `DASHBOARD`, `PLANNING`, `ARRIVEES`, `EN_CHAMBRES`, `DEPARTS`, `ATTRIBUER_CHAMBRE`, `MESSAGES`, `FACTURATION`,
        `CLOTURE`, `RAPPORT_RECEPTION`, `CARDEX`, `NOUVELLE_RESERVATION`, `MODIFIER_RESERVATION`, `FICHE_DE_POLICE`, `DISPONIBILITE_ET_TARIFS`, `PLAN_DE_CHAMBRE`, `RAPPORT_RESERVATION`, 
        `STATUTS_DES_CHAMBRES`, `HISTORIQUES_DES_CHAMBRES`, `HORS_SERVICES`, `OBJETS_TROUVES_PERDUS`, `RAPPORT_SERVICE_ETAGE`, `CLIENT_EN_CHAMBRE_FACTURATION`, `PAYMASTER_FACTURATION`, 
        `AU_COMPTANT_FACTURATION`, `RAPPORT_BAR_RESTAURANT`, `GESTION_DES_COMPTES`, `LISTE_DES_COMPTES`, `RECHARGE`, `CAUTIONS`, `RAPPORT_COMPTABILITE`, `MOUVEMENT`, `INVENTAIRE`, `FICHE_DE_PRODUIT`, 
        `FOURNISSEURS`, `RAPPORT_ECONOMAT`, `PETITE_CAISSE`, `GRANDE_CAISSE`, `PETIT_MAGAZIN`, `GRAND_MAGAZIN`, `SESSION_ADMIN`, `CONFIGURATION`, `SERVICE_TECHNIQUE`, `SECURITE`,`MENU_RECEPTION`, 
        ...
        `DEMANDE_INTERVENTION`, `INTERVENTION`, `RAPPORT_TECHNIQUE`, `RAPPORT_PROMOTEUR`,`RECHERCHER_RESA`,`NETTOYAGE`,`DEBUT_NETTOYAGE`,`FIN_NETTOYAGE`,`CONTROLLER_NETTOYAGE`,`ETAT_CHAMBRE`,
        `BON_COMMANDE`, `FICHE_TECHNIQUE`,`LISTE_DES_BONS`,`BON_RECEPTION`, `VALIDER`,`CONTROLER`, `COMMANDER`, `MAGASINS`,`VERIFICATION`, `CAISSE_PRINCIPALE_LECTURE`, `CAISSE_PRINCIPALE_ECRITURE`,
        `FACTURES_AGEES`,`FACTURES_COMPTABILITE`,`CAISSE_PRINCIPALE`,`LETTRE_RELANCE`,`GESTION_BLOC_NOTES`,`APPROVISIONNEMENT`,`CORRECTIONS`, `FISCALITE`, `MENU_CUISINE`,`IMPRIMER_FB`,
        `GRATUITEE_HEBERGEMENT`, `CAISSE_ENREGISTREUSE`, `FAST_FOOD`, `SERVEUR`, `BAR_FAST_FOOD`, `RESTAURANT_FAST_FOOD`, `MENU_MARKETING`, `MARK_CARDEX`, `MARK_RAPPORT`, `MARK_DASH`, `MARK_MESSAGERIE`, 
        `MARK_PLANNIFICATION`,`MARK_CLUB_ELITE`)
        VALUES (@CODE_PROFIL, @NOM_PROFIL, @value2,@value3, @value4, @value5, @value6, @value7, @value8, @value9,@value10,@value11,@value12,@value13,@value14,@value15,@value16,@value17,@value18,@value19,
        @value20,@value21,@value22,@value23,@value24,@value25,@value26,@value27,@value28,@value29,@value30,@value31,@value32,@value33,@value34,@value35,@value36,@value37,@value38,@value39,@value40,@value41,
        @value42,@value43,@value44,@value45,@value46,@value47,@value48,@value49,@value50,@value51,@value52,@value53,@value54,@value55,@value56,@value57,@value58,@value59,@value60,@value61,@value62,@value63,
        @value64,@value65,@value66,@value67,@value68,@value69,@value70,@value71,@value72,@value73,@value74, @VERIFICATION, @CAISSE_PRINCIPALE_LECTURE, @CAISSE_PRINCIPALE_ECRITURE, @FACTURES_AGEES, 
        @FACTURES_COMPTABILITE, @CAISSE_PRINCIPALE, @LETTRE_RELANCE, @GESTION_BLOC_NOTES, @APPROVISIONNEMENT, @CORRECTIONS, @FISCALITE, @MENU_CUISINE, @IMPRIMER_FB, @GRATUITEE_HEBERGEMENT
        ,@CAISSE_ENREGISTREUSE,@FAST_FOOD,@SERVEUR,@BAR_FAST_FOOD,@RESTAURANT_FAST_FOOD,@MENU_MARKETING,@MARK_CARDEX,@MARK_RAPPORT,@MARK_DASH,@MARK_MESSAGERIE, 
       @MARK_PLANNIFICATION,@MARK_CLUB_ELITE)"
        ...
        command.Parameters.Add("@CAISSE_ENREGISTREUSE", MySqlDbType.Int16).Value = CAISSE_ENREGISTREUSE
        command.Parameters.Add("@FAST_FOOD", MySqlDbType.Int16).Value = FAST_FOOD
        command.Parameters.Add("@SERVEUR", MySqlDbType.Int16).Value = SERVEUR

        command.Parameters.Add("@BAR_FAST_FOOD", MySqlDbType.Int16).Value = BAR_FAST_FOOD
        command.Parameters.Add("@RESTAURANT_FAST_FOOD", MySqlDbType.Int16).Value = RESTAURANT_FAST_FOOD

        command.Parameters.Add("@MARK_CARDEX", MySqlDbType.Int16).Value = MARK_CARDEX
        command.Parameters.Add("@MARK_RAPPORT", MySqlDbType.Int16).Value = MARK_RAPPORT
        command.Parameters.Add("@MARK_DASH", MySqlDbType.Int16).Value = MARK_DASH
        command.Parameters.Add("@MARK_MESSAGERIE", MySqlDbType.Int16).Value = MARK_MESSAGERIE
        command.Parameters.Add("@MARK_PLANNIFICATION", MySqlDbType.Int16).Value = MARK_PLANNIFICATION
        command.Parameters.Add("@MARK_CLUB_ELITE", MySqlDbType.Int16).Value = MARK_CLUB_ELITE
        command.Parameters.Add("@MENU_MARKETING", MySqlDbType.Int16).Value = MENU_MARKETING
    ...
    Public Function UpdateAccessRight(ByVal CODE_PROFIL As String, ByVal NOM_PROFIL As String,
        ...
        ByVal BAR_FAST_FOOD As Integer,
        ByVal RESTAURANT_FAST_FOOD As Integer,
        ByVal OLD_CODE_PROFIL As String,
         ByVal MENU_MARKETING As Integer,
        ByVal MARK_CARDEX As Integer,
        ByVal MARK_RAPPORT As Integer,
        ByVal MARK_DASH As Integer,
        ...
        FISCALITE=@FISCALITE, MENU_CUISINE=@MENU_CUISINE, IMPRIMER_FB=@IMPRIMER_FB, GRATUITEE_HEBERGEMENT=@GRATUITEE_HEBERGEMENT, CAISSE_ENREGISTREUSE=@CAISSE_ENREGISTREUSE, FAST_FOOD=@FAST_FOOD,
        SERVEUR=@SERVEUR, BAR_FAST_FOOD=@BAR_FAST_FOOD, RESTAURANT_FAST_FOOD=@RESTAURANT_FAST_FOOD, MENU_MARKETING=@MENU_MARKETING, MARK_CARDEX=@MARK_CARDEX, MARK_RAPPORT=@MARK_RAPPORT, MARK_DASH=@MARK_DASH, 
        MARK_MESSAGERIE=@MARK_MESSAGERIE, MARK_PLANNIFICATION=@MARK_PLANNIFICATION, MARK_CLUB_ELITE=@MARK_CLUB_ELITE
